...
export function spollers() {
  ...
  if (spollersArray.length > 0) {
    const spollersRegular = Array.from(spollersArray).filter(
      (item) => !item.getAttribute("data-spollers-media")?.split(",")[0],
    );
    if (spollersRegular.length) {
      initSpollers(spollersRegular);
    }

    const mdQueriesArray = dataMediaQueries(
      spollersArray,
      "data-spollers-media",
    );
    ...
    function initSpollerButton(spollersBlock, hideSpollerBody = true) {
      let spollerButtons = spollersBlock.querySelectorAll(
        "[data-spoller-button]",
      );
      const spollersBlockName = spollersBlock.getAttribute("data-spollers");
      if (spollerButtons.length) {
        spollerButtons = Array.from(spollerButtons).filter(
        ...
        spollerButtons.forEach((spollerButton, index) => {
          if (hideSpollerBody) {
            ...
            if (!spollerButton.classList.contains("spollers__button--active")) {
              ...
              spollerButton.nextElementSibling.setAttribute(
                "id",
                `${spollersBlockName}-${index}`,
              );
              spollerButton.nextElementSibling.setAttribute(
                "aria-labelledby",
                `${spollersBlockName}-${index}`,
    ...
    function initSpollerTitle(spollersBlock, hideSpollerBody = true) {
      let spollerTitles = spollersBlock.querySelectorAll(
        "[data-spoller-title]",
      );
    ...
    function setSpollerAction(e) {
      ...
      if (el.closest("[data-spoller-button]")) {
        ...
        if (!spollersBlock.querySelectorAll("._slide").length) {
          if (
            oneSpoller &&
            !spollerButton.classList.contains("spollers__button--active")
          ) {
    ...
    function hideSpollersBody(spollersBlock) {
      const spollerActiveTitle = spollersBlock.querySelector(
        "[data-spoller-button].spollers__button--active",
      );
